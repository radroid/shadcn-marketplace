...
function MonacoEditorInternal({ readOnly, onChange }: MonacoEditorInternalProps) {
    const { code, updateCode } = useActiveCode();
    const { sandpack } = useSandpack();
    const { theme } = useTheme();

    // Listen to file changes and trigger onChange
    React.useEffect(() => {
        if (onChange) {
...
export default function ComponentEditor({ code, previewCode, globalCss, readOnly, onChange, dependencies, componentName }: ComponentEditorProps) {
...
module.exports = {
  ...
  theme: {
    ...
    return (
        ...
        >
            <SandpackLayout>
                <MonacoEditorInternal readOnly={readOnly} onChange={onChange} />
